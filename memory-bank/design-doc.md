...
> V1.2 信息架构修正：当前版本主流程统一为“输入想法 → 需求澄清 → 需求确认 → 生成 PRD”。架构建议、接口设计、数据库表设计和流程图不再作为一级必经模块，也不再阻塞 PRD 生成；它们作为 PRD 生成后的可选技术方案或需求卡片级附件保留。PRD 主文档章节收缩为产品目标、用户角色、功能范围、用户故事、业务规则、异常场景、页面清单、数据需求、验收标准、非功能需求、风险与待确认事项。
...
AI-ready PRD 的主要下游使用者是 Codex、Claude Code、Cursor 等 coding agent，而不只是人工产品或开发人员。文档需要让 AI 明确知道“做什么、不要做什么、遵守哪些技术决策和工程约束、按什么顺序实现、怎样判断完成”，从而减少用户在 vibe coding 过程中反复补充上下文和修正错误方向。
...
- 降低 AI 编程偏航：生成明确的技术决策摘要、数据模型、目录建议、实施阶段和禁止事项。
...
- 根据用户回答实时更新结构化需求卡片。
- 识别需求之间的潜在冲突并要求用户确认。
- 展示当前阶段、待处理事项和仍待确认的内容。
- 在需求卡片上提供可选的验收标准与流程图生成入口，只为用户选中的复杂需求生成流程图。
- 在 PRD 生成后提供可选的技术方案入口，根据用户技能、项目规模、部署条件和成本约束提供架构候选方案及推荐结论。
- 生成包含固定需求章节的 PRD，并导出为 Markdown 文件。
...
用户完成需求和架构确认后导出 AI-ready PRD，将其作为上下文提供给 Codex、Claude Code、Cursor 等工具。Coding agent 可以从文档中读取技术决策摘要、数据模型、接口契约、实施顺序和验收条件，而不需要用户重新口头解释项目。完整架构比较、评分矩阵、部署拓扑和系统设计说明应进入独立的《技术方案》或《系统设计说明书》，不塞进 PRD 正文。

## 6. 核心产品原则

### 6.1 先澄清，后生成

系统不得在核心需求明显不足时直接生成 PRD 或引导生成草稿。用户可以主动跳过单题或整轮问题，但在需求完整度未达到生成条件时，系统应继续生成下一轮澄清问题；文档必须明确标记未经确认的假设。
...
- 提供 DeepSeek、OpenAI 和其他 OpenAI 兼容服务三类配置入口。
- 测试当前模型配置是否可用。
...
系统至少从以下维度检查需求，并通过多轮问题逐步补齐最终 PRD 的最低信息框架：
...
完整度用于内部排序和缺口判断，不再作为顶部主状态展示。需要查看详情时，可展示各维度得分，至少包括：
...
达到生成条件后，系统提供“生成 PRD”作为唯一主操作，不自动开始生成，也不强制结束需求澄清。用户仍可继续补充需求；未达到条件时，按钮附近只显示下一步原因，例如“确认全部需求后即可生成 PRD”。

### 9.5 流程图生成

#### FR-401 生成主业务流程

系统只在用户从需求卡片主动选择复杂功能后生成 Mermaid 业务流程图。图中必须标明流程起点、关键决策、主要状态和结束条件，避免只罗列功能模块。

#### FR-401A 生成异常流程

系统根据用户选中需求相关的已确认异常场景生成一张或多张 Mermaid 异常流程图，至少说明异常的触发条件、责任角色、处理动作、状态变化和结束结果。没有已确认异常场景时，界面应提示用户先补充需求，而不是由模型虚构事故处理规则。
...
最终 PRD 的技术决策摘要章节只保留已确认且会约束实现的内容，至少包含：

- 已确认方案名称或架构 ID
- 技术栈与关键工程约束
- 前端、后端、存储、AI 接入、测试和部署的简要边界
- 与需求实现直接相关的限制和禁止事项

该章节不得展开架构比较、评分矩阵、备选方案细节、部署拓扑图或完整系统设计说明。需要完整架构说明时，应单独生成《技术方案》或《系统设计说明书》。
...
系统通过统一的 OpenAI 兼容客户端接入模型服务，并允许配置服务地址和模型名称。第一版提供 DeepSeek、OpenAI 和其他 OpenAI 兼容服务三类入口；DeepSeek 与 OpenAI 自动使用预设地址，其他兼容服务允许用户填写 Base URL，不把需求分析逻辑绑定到单一模型。
...
作为用户，我希望知道当前需求是否足以进入开发，以及还缺少哪些信息；当信息不足时系统应继续追问，而不是让我进入低完整度草稿页。
...
- BR-040：最终 PRD 只能包含一个已确认主架构对应的技术决策摘要，其他候选方案、评分矩阵和未选择原因不得进入 PRD 实施指令。
...
需求完整度未达到生成条件或仍有核心冲突时，问题向导不得展示“先生成 PRD 草稿”的中间页，也不得把生成 PRD 作为主操作。若当前没有待答问题，系统应自动请求下一轮澄清问题；若模型仍未返回问题，则提示用户补充想法或查看需求结果。达到标准后也只提示用户生成，不自动触发生成任务。
...
只读取当前有效需求状态和已确认架构的技术决策摘要，按固定章节分别生成 AI-ready PRD。文档生成结果不反向修改正式需求状态，也不把架构比较或完整系统设计写入 PRD。

#### 一致性检查器

在导出前检查需求编号、字段名称、状态枚举、接口引用、技术决策摘要、实施阶段和验收条件是否一致。发现冲突时阻止文档被标记为最终版本，并指出需要修复的章节。
...
当各维度得分发生变化时，系统应按照规定权重计算内部缺口评分；核心业务流程和功能模块保持较高权重，数据需求、页面清单、验收标准和风险事项按文档规定计算。不适用维度从权重分母中排除。该评分用于问题排序和缺口提示，不作为 PRD 生成的主阻塞条件。
...
当用户已经生成 PRD 后，可以主动进入“生成技术方案（可选）”。系统应先询问已有技术栈、团队规模、开发周期、部署环境、预算、并发量和数据敏感程度，再生成技术建议；没有技术方案时不得阻塞 PRD 生成。

### AC-037 AI-ready PRD

当用户导出最终 PRD 时，文档应包含稳定需求编号、产品目标、用户角色、功能范围、用户故事、业务规则、异常场景、页面清单、数据需求、验收标准、非功能需求和风险待确认事项。任一核心功能都应能追溯到对应业务规则和验收条件；技术方案、接口设计、数据库表设计和流程图作为可选附件。
...
10. 达标后允许继续回答或生成 PRD；未达标且没有待答问题时自动继续生成下一轮澄清问题。
