...
**目标：** 交付一个本地优先的 Prompt2PRD MVP，使用户能够从模糊需求出发，完成动态澄清、结构化状态管理、PRD 编辑与 Markdown 导出。当前主流程统一为“输入想法 → 需求澄清 → 需求确认 → 生成 PRD”；架构建议和流程图作为可选工具保留，不再作为主流程必经步骤或 PRD 生成条件。

**2026-07-18 信息架构修正：** 后续实施以四步主流程为准。原计划中架构确认、流程图生成、接口设计、数据库表设计和工程实施章节相关任务，应作为“更多工具”或 PRD 生成后的可选附件处理；不得重新引入架构选择或流程图生成对 PRD 生成的阻塞。需求完整度未达 80% 或仍有核心冲突时，问题向导应继续追问，不展示低完整度 PRD 草稿页。
...
- **指令：** 只读取当前有效需求和已确认架构；按章节分别生成，不允许文档结果反向修改需求状态；未确认架构时可以明确标为可选技术内容缺失。前端不得在完整度不足或仍有核心冲突时把用户引入 PRD 生成页。
- **验证：** 测试最终文档、未确认架构提示和核心冲突场景；确认最终模式被阻止时已有需求状态不变化。
